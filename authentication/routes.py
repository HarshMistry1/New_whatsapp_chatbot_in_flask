...
from login_panel import db, login_manager
...
@blueprint.route('/new_login', methods=['GET', 'POST'])
def new_login():
    login_form = LoginForm(request.form)
    if 'new_login' in request.form:
        ...
        if not user:
            ...
            if not user:
                return render_template('accounts/new_login.html',
                                        msg='Unknown User or Email',
                                        form=login_form)
    ...
    if not current_user.is_authenticated:
        return render_template('accounts/new_login.html',
                               form=login_form)
    return redirect(url_for('home_blueprint.new_index'))
